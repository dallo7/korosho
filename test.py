...
class PDF(FPDF):
    ...
    def header(self):
        # --- 3-LOGO HEADER ---
        try:
            self.image(self.cp_logo_path, 10, 8, 33)  # Capital Pay Logo (Left)
        except Exception:
            self.cell(40, 10, '[CP Logo]')
        try:
            self.image(self.korosho_logo_path, 45, 8, 12)  # Korosho-JE Logo (Left)
        except Exception:
            self.cell(60)
            self.cell(40, 10, '[K Logo]')
        try:
            self.image(self.nbc_logo_path, 170, 8, 30)  # NBC Logo (Right)
        except Exception:
        ...
        self.set_xy(100, 10)
        self.cell(65, 10, f'VERIFICATION {self.doc_type}', 0, 1, 'R')

        self.set_font('Arial', '', 10)
        self.set_text_color(85, 85, 85)
        self.set_xy(100, 17)
        self.cell(65, 5, 'Capital Pay / KOROSHO-JE / NBC', 0, 1, 'R')
    ...
    def footer(self):
        ...
        try:
            ...
            pass  # Fail silently if logo is missing
        # --- End of Footer Update ---

    def add_bill_to_section(self):
        """Helper to add the 'BILL TO' and 'INVOICE #' block."""

        # --- Store Y position to create two columns ---
        current_y = self.get_y()

        # --- Column 1: BILL TO (Left-Aligned) ---
        self.set_font('Arial', '', 9)
        self.set_text_color(119, 119, 119)
        self.cell(100, 5, 'BILL TO', 0, 1, 'L')  # Add new line

        self.set_font('Arial', 'B', 12)  # Slightly larger font
        self.set_text_color(0, 0, 0)
        self.cell(100, 7, self.coop_name, 0, 1, 'L')  # Add new line

        # --- Column 2: RECEIPT DETAILS (Right-Aligned) ---
        self.set_y(current_y)  # Reset Y to the top

        # --- Row 1: Receipt # ---
        self.set_x(130)  # Start of the right column
        self.set_font('Arial', '', 9)
        self.set_text_color(119, 119, 119)
        self.cell(30, 7, f'{self.doc_type} #:', 0, 0, 'R')  # Right-aligned label

        self.set_font('Arial', 'B', 11)
        self.set_text_color(0, 0, 0)
        self.cell(40, 7, self.ref, 0, 1, 'L')  # Left-aligned value

        # --- Row 2: Date ---
        self.set_x(130)  # Start of the right column
        self.set_font('Arial', '', 9)
        self.set_text_color(119, 119, 119)
        self.cell(30, 7, 'DATE:', 0, 0, 'R')  # Right-aligned label

        self.set_font('Arial', 'B', 11)
        self.set_text_color(0, 0, 0)
        self.cell(40, 7, self.date_str, 0, 1, 'L')  # Left-aligned value

        self.ln(10)  # Add space after the block
...
def generate_service_invoice_pdf(ref, date, coop_name, rows, amount, status):
    ...
    pdf.cell(45, 10, 'TOTAL DUE', 0, 0, 'R')
    ...
    # --- NEW PAYMENT INSTRUCTIONS BLOCK ---
    ...
    pdf.set_xy(100, current_y)  # Move to the right column
    pdf.set_font('Arial', 'B', 10)
    pdf.set_fill_color(244, 244, 244)
...
def generate_payment_receipt_pdf(ref, date, coop_name, total_tsh, commission_usd):
    ...
    pdf.set_font('Arial', '', 14)
    pdf.set_text_color(85, 85, 85)
    pdf.cell(45, 10, 'TOTAL FEE PAID (USD)', 0, 0, 'R')
    pdf.set_font('Arial', 'B', 16)
    pdf.set_text_color(0, 0, 0)
    ...
    # --- NEW RECEIPT ELEMENTS ---
    # Add a "PAYMENT RECEIVED" stamp near the total
    current_y = pdf.get_y()
    pdf.set_font('Arial', 'B', 16)
    pdf.set_text_color(40, 167, 69) # Green
    pdf.set_xy(10, current_y - 10) # Position it to the left of the total
    pdf.cell(50, 10, "PAYMENT RECEIVED", 0, 1, 'C')

    # Add disclaimer and thank you note (positioned from bottom)
...
def create_invoice_modal_layout(ref, date, coop_name, rows, amount, status, logos):
...
def create_receipt_modal_layout(ref, date, coop_name, total_tsh, commission_usd, logos):
...
def render_admin_invoices(active_tab, trigger):
    ...
    df['amount_usd_raw'] = df['amount_usd']  # --- Store raw value for modal ---
    ...
    # REMOVED: df['action'] = "View Invoice"
    ...
    style_data_conditional = [
        ...
        # REMOVED: Style rule for 'Action' column
    ]

    # REMOVED: 'Action' from this list
...
def render_payment_history(active_tab, ipn_data, trigger):
    ...
    with get_db_connection() as conn:
        query = """
            SELECT p.processing_timestamp, p.cooperative_name, p.filename, p.record_count, p.total_amount, 
    ...
    df['processing_timestamp'] = pd.to_datetime(df['processing_timestamp']).dt.strftime('%Y-%m-%d %I:%M %p')
    df['total_amount_raw'] = df['total_amount']
    df['total_amount'] = df['total_amount'].apply(lambda x: f"TSH {x:,.2f}" if pd.notna(x) else "N/A")
    df['payment_commission_usd_raw'] = df['payment_commission_usd']
    df['payment_commission_usd'] = df['payment_commission_usd'].apply(lambda x: f"${x:,.2f}" if pd.notna(x) else "N/A")
    df['action'] = "View Receipt"

    df = df.rename(columns={
        'processing_timestamp': 'Date Processed', 'cooperative_name': 'Cooperative', 'filename': 'Filename',
        'record_count': 'Records', 'total_amount': 'Total Payout (TSH)',
        'payment_commission_usd': 'Commission (USD)', 'payment_commission_reference': 'Ref #',
        'action': 'Action'
    })
    display_cols = ['Date Processed', 'Ref #', 'Cooperative', 'Filename', 'Records', 'Total Payout (TSH)',
                    'Commission (USD)', 'Action']
    ...
    return dash_table.DataTable(
        ...
        style_cell={'textAlign': 'left', 'whiteSpace': 'normal', 'height': 'auto'},
        style_data_conditional=[
            {'if': {'column_id': 'Action'}, 'color': 'blue', 'textDecoration': 'underline', 'cursor': 'pointer'},
            {'if': {'column_id': 'Commission (USD)'}, 'backgroundColor': '#e8f5e9', 'fontWeight': 'bold'}
        ],
        filter_action="native",
        sort_action="native",
        export_format="csv",  # --- ADDED ---
...
@app.callback(
    ...
    # REMOVED: Input("admin-invoice-table", "active_cell"),
    Input({'type': 'admin-download-modal-close', 'index': ALL}, "n_clicks"),
    State("payment-history-table", "data"),
    # REMOVED: State("admin-invoice-table", "data"),
    State("admin-download-modal", "is_open"),
    prevent_initial_call=True
)
def toggle_admin_download_modal(receipt_cell, close_clicks, receipt_data, is_open):
    ctx = callback_context
    ...
    if triggered_id == "payment-history-table" and receipt_cell and receipt_cell['column_id'] == 'Action':
        row_data = receipt_data[receipt_cell['row']]
        raw_total_tsh = row_data.get('total_amount_raw', 0)
        raw_commission_usd = row_data.get('payment_commission_usd_raw', 0)

        modal_content = create_receipt_modal_layout(
            ref=row_data['Ref #'],
            date=row_data['Date Processed'],
            coop_name=row_data['Cooperative'],
            total_tsh=raw_total_tsh,
            commission_usd=raw_commission_usd,
            logos=ASSET_URLS  # <-- THIS LINE IS THE FIX
        )
        download_data = {
